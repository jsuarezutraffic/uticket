...
const notificarTicket = async (cliente) => {
  ...
  var plantilla =
    require("../components/PlantillaCorreo.html").default.toString();
...
const correoOperador = async (data, idTiquete) => {
  data.estado = "Actualizado";
  data.mensaje1 = `El ticket N° ${idTiquete} fue actualizado por el cliente.`;
  data.mensaje2 = "Por favor verificar y realizar la accion necesaria";
...
const correoBackoffice = async (data) => {
  data.estado = "Creado";
  data.mensaje1 = `Un ticket fue creado y asignado automaticamente a usted.`;
  data.mensaje2 =
    "Por favor verificar el ticket y realizar la acción necesaria";

  const dominio = "https://uticket.bak.utraffic.co/";
  var accion = `Ticket ${data.estado} Exitosamente!`;
  var mensaje = `${data.mensaje1} <strong>Utraffic SAS.</strong><br> ${data.mensaje2}`;
  var plantilla =
    require("../components/PlantillaCorreo.html").default.toString();
